...
```plaintext
curl -X POST -H "Accept: text/plan" -H "X-authorization:<>" -d "uri=<>&value=<>" http://synbiohub.org/updateMutableNotes
```

## Edit Source

```plaintext
curl -X POST -H "Accept: text/plan" -H "X-authorization:<>" -d "uri=<>&value=<>" http://synbiohub.org/updateMutableSource
```

## Edit References

```plaintext
curl -X POST -H "Accept: text/plan" -H "X-authorization:<>" -d "uri=<>&value=<>" http://synbiohub.org/updateCitations
```
